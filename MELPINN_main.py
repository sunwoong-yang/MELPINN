...
def save_masses(net):
	...
	text_to_save = (
		f"Tank 1 mass: {final_results[5]}\n"
		f"Tank 2 mass: {final_results[6]}\n"
		f"Tank 3 mass: {final_results[7]}\n"
		f"Tank 4 mass: {final_results[8]}\n"
		f"Tank 5 mass: {final_results[9]}\n"
		f"Tank 6 mass: {final_results[10]}\n"
	)

	# Save text to a file
	with open('tank_masses.txt', 'w') as file:
		file.write(text_to_save)

	print(f"Tank 1 mass: {final_results[5]}")
	print(f"Tank 2 mass: {final_results[6]}")
	print(f"Tank 3 mass: {final_results[7]}")
	print(f"Tank 4 mass: {final_results[8]}")
	print(f"Tank 5 mass: {final_results[9]}")
	print(f"Tank 6 mass: {final_results[10]}")
...
def MELCOR_pde(x, u):
	...
	# print('111', torch.sum(L * du1_dt))
	# print('222', torch.sum(- g * h1))
	# print(mass[:, [0]])
	...
	continuity1 = dm1_dt + rho * vel[:, [0]] * A_p * (1 - a_o1)
	continuity2 = dm2_dt + rho * vel[:, [1]] * A_p * (1 - a_o2) - rho * vel[:, [0]] * A_p * (1 - a_o1)
	continuity3 = dm3_dt + rho * vel[:, [2]] * A_p * (1 - a_o3) - rho * vel[:, [1]] * A_p * (1 - a_o2)
	continuity4 = dm4_dt + rho * vel[:, [3]] * A_p * (1 - a_o4) - rho * vel[:, [2]] * A_p * (1 - a_o3)
	continuity5 = dm5_dt + rho * vel[:, [4]] * A_p * (1 - a_o5) - rho * vel[:, [3]] * A_p * (1 - a_o4)
	continuity6 = dm6_dt - rho * vel[:, [4]] * A_p * (1 - a_o5)
...
def hard_IC(x, y):
	...
	for y_idx in range(y.shape[1]):
		if y_idx == 5:
			# mass_tank1 = 100000. - x[:, [1]] * y[:, [y_idx]] # 100000 - t * mass of tank5
			others.append(100000. - x[:, [1]] * y[:, [y_idx]])  # 100000 - t * mass of tank5
		else:
			others.append(x[:, [1]] * y[:, [y_idx]])  # t * mass of tank except 5
...
model.compile("adam", lr=1e-3, loss_weights=[1] * 11, decay=("step", 250, 0.92))

model.train(iterations=1000, display_every=300)
final_results = save_masses(net)
# asdf = torch.tensor([[0.5, 2757]], dtype=torch.float32)
asdf = torch.tensor([[0.5, 2]], dtype=torch.float32)
final_results = net(asdf).squeeze()
