...
print(scheduler_step, scheduler_gamma)
...
def residual(net, t):
	...
	momentum = [L * du_dt[i] - g * height[:, [i]] + K * torch.abs(vel[:, [i]]) * vel[:, [i]] / 2 for i in
	            range(N_tank - 1)]
...
def train_pinn(weights=None, use_hard_BC=True, epochs=30000, N_collo=2000, end_time=2758, plot_period=1000,
	...
	net = PINN(hard_BC=hard_BC_func, hidden_layers=hidden_layers).to(device)
...
def plot_velocities(net, t_end=3000, epoch=None):
	...
	plt.figure(figsize=(10, 6))
	# colors_vel = plt.cm.plasma(np.linspace(0, 1, N_tank - 1))
