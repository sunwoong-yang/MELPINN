...
plt.plot(iteration, V_2, label='Tank 2 Velocity', alpha=0.6, linewidth=2.5)
...
# Save the figure
plt.savefig("figures/MELCOR_ThreeTanks_velocity.png", dpi=300)
# I would like to add 9 to all the values of H1
H1 = [x + elev*2 for x in H1]
H2 = [x + elev*1 for x in H2]
H3 = [x + elev*0 for x in H3]
#
# Create a dataframe: and write in the Excel file
df = pd.DataFrame({'Time' : iteration, 'H1': H1, 'H2' : H2, 'H3' : H3,
                   'V_1': V_1, 'V_2': V_2})
